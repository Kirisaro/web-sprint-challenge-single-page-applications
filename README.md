...
1. In 1-2 sentences, explain what React's `useRouteMatch` hook is used for. useRouteMatch hook is used for routing so you can get access to data without rendering.
1. How would you explain form validation to someone who has never programmed before? Form validation is a a process that checks if the user input was provided correctly, if it wasn't it will alert the user they need to fix something to proceed.
1. In 1-2 sentences, define end to end testing. End to end testing tests the functionality and performance of the app, it simulates what a real user would see from the first to last interaction.
